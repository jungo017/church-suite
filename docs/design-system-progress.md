...
- [ ] 남은 미전환(D2/D3 후속): `site/offerings`·`site/new-family`·`site/boards/[boardId]`·`my/forms`·`members/kiosk`(raw green/blue 색상), `app/platform`(raw table)
- [x] 품질게이트: typecheck ✅ / lint ✅ / build ✅ / test (재실행 중)
...
- [x] 실데이터 시각 확인(manualdemo 공개 홈): 콘솔 에러 0, 내부 앱과 톤 분리
- [ ] 후속: `p/[slug]`·`b/[slug]/[postId]`·online 폼(new-family/offering) 컴포넌트 적용은 남음(헤더는 재노출로 이미 개선됨)
...
- [x] 주요 화면 시각 확인(members/finance 내부 앱, 공개 홈)
- [ ] 금지색 규칙 lint 자동화(현재는 리뷰 체크리스트 수준)
- [ ] 전 역할별 화면 점검

## D3. 폼/상세 — ⬜ 미착수(후속)

상세/등록·수정 폼의 `Field`·`DescriptionList`·위험액션 분리는 다음 작업으로 남김.
(D0 에서 `Field` 프리미티브는 준비됨 — 적용만 남음)
...
- 남은 범위: D2 잔여 5파일·platform table, D3 전체, D4 잔여 페이지, D5 lint 자동화.
