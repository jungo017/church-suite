...
- [x] 폼/응답 상세 `DescriptionList`: `forms/[formId]`·`responses/[responseId]`·`my`·`my/forms/[assignmentId]`
- [x] 잔여 (app) 페이지 21개 일괄 표준화 — PageHeader/PageTitle 전면 적용((app) 영역 구식 `<h1>` 0개)
...
- 남은작업 라운드2 — (app) 잔여 21페이지 일괄 표준화(5개 병렬 에이전트): members 보조(attendance·compliance·families·stats·education·education/[id]·labels·notify·org), assets(audits·audits/[id]·classification·labels)+dashboard, forms 상세(formId·responses·responses/[id])+my·my/forms/[id], site·site/pages/[id]. PageHeader/PageTitle 전면 적용, EmptyState·Badge·DescriptionList·Field 확산.
- **결과(전체): (app) 구식 `<h1>` 0개, raw `<table>` 0건, 금지색 0건(lint 강제), typecheck/lint/build green, 테스트 101 pass(storage-usage 단독 재실행 통과 — round3 동시 build 경합 flaky).**
- 남은 범위(후속): 역할별(staff/viewer/member) 화면 점검.
