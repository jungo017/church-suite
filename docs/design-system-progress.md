...
- [ ] `app/platform`(raw table) — 플랫폼 대시보드는 별도 기능, 후속
...
- [x] 실데이터 시각 확인(교인 상세)
- [ ] 후속: 전표/폼 빌더 상세, 등록·수정 폼 전반 `Field` 적용, `DescriptionList` 확산
...
- 남은 범위: D3 등록·수정 폼 `Field` 확산, `app/platform` table, D4 잔여 공개 페이지, D5 lint 자동화.
