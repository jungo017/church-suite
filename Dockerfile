...
# Node 22 LTS — pnpm 11 은 node:sqlite(Node 22+ 내장)를 사용하므로 Node 20 불가. CI(setup-node 22)와 일치.
FROM node:22-bookworm-slim AS base
WORKDIR /app
# pnpm 워크스페이스(스펙 §1 P-1). corepack 대신 전역 설치 — 결정적(corepack 다운로드/번들 이슈 회피).
