...
- **디자인 시스템 재적용 진행(DESIGNE.md · 추적 [`docs/design-system-progress.md`](./docs/design-system-progress.md)):** 1차 시도(PR #27)는 모듈 마이그레이션과 충돌로 닫음 → 청사진 삼아 현재 main 위에 단계별 재적용. **D0(기반) 완료:** deps(cva·radix-slot·lucide), `globals.css` 상태토큰(success/warning/info×4테마), `lib/ui` 프리미티브 업그레이드(button cva·card 서브컴포넌트·badge 토큰·form Field) + 신규(table·empty-state·skeleton·page=PageHeader), login 적용. **D1(셸 분해) 완료:** `lib/ui/app/*`(types·module-nav·app-sidebar) + `app-shell` 반응형/모바일 subnav/lucide(현 M2 셸 props 유지, layout 무변경). 122 tests green. **다음:** D2(목록/Table)·D3(폼/상세)·D4(공개)·D5(색상 lint+QA).
