...
- **모듈 플랫폼 마이그레이션 진행(스펙 §1 P-1 · 상세 [`module-platform.md`](./module-platform.md) §10):** M0a/M0b/M1/M2/M3/M1.5 → **M3.5(코어 추출 2회차: billing/compliance/jobs/notify→코어) 완료.** 이제 members/finance/assets/site 는 **코어만** 의존. 다음: **M4**(모듈 패키지 물리 추출 `packages/module-*`) — **선행 디커플링 결정 필요(미착수):** ① `department`(코어 공유) CRUD 가 `lib/assets/classification` 에 있어 members 라우트가 assets 참조 → department CRUD 코어 이전, ② `forms`→`members`(org·portal) 모듈 간 직접 결합 → 코어 readContract(파라미터 확장) 치환. 이후 **M5**(`public`→모듈 Postgres 스키마, 선택). A안(모듈러 모놀리식·단일 배포) 유지.
