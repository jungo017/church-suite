...
- **단계: Phase 0(코어) — 작업 `0.1 스캐폴드` 착수 직전.**
- 리포지토리는 아직 그린필드: 스펙 문서와 이 문서만 존재. **코드/패키지/마이그레이션 없음.**
- 따라서 아래 "명령어" 섹션의 항목 대부분은 **0.1 스캐폴드 완료 후** 동작합니다. 스캐폴드 시 이 섹션과 §5를 실제 값으로 갱신할 것.
...
> 0.1 스캐폴드 전이라 아직 실행 불가. 스캐폴드 완료 시 실제 스크립트로 교체할 것.
...
# DB / 마이그레이션 (Drizzle)
npm run db:generate  # 스키마 → 마이그레이션 생성
...
npm run lint
npm run typecheck
npm run test         # 격리/권한 테스트 포함
```

**환경 요건:** Node 20+ (Next 16). PostgreSQL. `.env`로 DB 연결/JWT 시크릿/스토리지 설정.
...
app/
  ...
  (app)/             # 인증 대시보드 (SSR) — members/ finance/ assets/ ...
middleware.ts        # 호스트 → church_id 해석·인증·컨텍스트 주입
lib/
  db/                # Drizzle 데이터 계층 + RLS 세션 변수 + 뷰/raw SQL
  auth/              # JWT/세션/리프레시
