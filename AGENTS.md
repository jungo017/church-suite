...
- **구현됨(소셜로그인→교인 매핑 스캐폴드 · §14, 자격증명 불필요분):** 매핑 seam = `app_user.member_id`. ① 스키마(코어): `user_identity`(provider/providerUserId↔app_user, church 스코프 unique)·`member_claim`(관리자 발급 1회용 토큰=sha256 해시) + `app_user.loginId`/`passwordHash` **nullable**(소셜 전용 계정) + RLS(0038/0039). ② 공급자 추상화 `@church/core/auth/oauth`(`OAuthProvider`/`OAuthProfile`, `OAUTH_DRIVER` 게이트, **mock 드라이버**=code(JSON)→프로필; kakao/naver 실드라이버는 키 확보 후). ③ `@church/core/auth/identity`(find/link)·`auth/member-claim`(issue/redeem — **A안 결정적 바인딩**: 비번없는 app_user 생성+member_id 연결+member 역할+클레임 소진)·`auth/session.loginWithIdentity`(미연결=`identity_not_linked`→클레임/새가족접수[C안] 안내). 비번 로그인은 passwordHash null 계정 차단. 격리·멱등·만료/중복/타교회/이미연결·매핑 테스트(130 tests). **미구현(키/결정 필요):** 실드라이버(카카오/네이버), OAuth 콜백 라우트(state/PKCE), 관리자 클레임 발급 UI(members:write), C안(새가족 intake) 연결, 이메일/전화 자동매칭(B안).
- **모듈 플랫폼 마이그레이션 진행(스펙 §1 P-1 · 상세 [`module-platform.md`](./module-platform.md) §10):** M0a~M3·M1.5·M3.5 → **M4 완료(5개 모듈 전부 패키지 추출).** `lib/{members,finance,assets,site,forms}` → `packages/module-*`(`@church/module-*`). **각 모듈은 `@church/core/*` 만 의존, 모듈→모듈 결합 0.** 라우트(`app/(app)/<m>`)·공개(`app/(public)`)는 단일배포라 앱에 잔류하고 패키지를 import. **boundary 강제 완료**(ESLint `no-restricted-imports` 로 `packages/**` 의 `@/*`·`@church/module-*` import 차단 — CI). **워커 server-only 수정 완료**(`pnpm run worker` = `tsx --conditions=react-server` → server-only no-op, 워커가 notify/remind import 가능; 앱 가드 유지). **M5(`public`→모듈 Postgres 스키마)는 보류 결정**(고위험·실익은 B안 전환 시 — B안 트리거 시 진행). **→ 모듈 플랫폼(A안) 완료.** A안(모듈러 모놀리식·단일 배포) 유지.
...
- **디자인 시스템 재적용 진행(DESIGNE.md · 추적 [`docs/design-system-progress.md`](./docs/design-system-progress.md)):** 1차 시도(PR #27)는 모듈 마이그레이션과 충돌로 닫음 → 청사진 삼아 현재 main 위에 단계별 재적용. **D0(기반) 완료:** deps(cva·radix-slot·lucide), `globals.css` 상태토큰(success/warning/info×4테마), `lib/ui` 프리미티브 업그레이드(button cva·card 서브컴포넌트·badge 토큰·form Field) + 신규(table·empty-state·skeleton·page=PageHeader), login 적용. **D1~D5 완료:** D1 셸 분해(`lib/ui/app/*`+반응형/모바일/lucide) · D2 핵심 목록/Table(+대시보드, 엔타이틀먼트 보존) · D3 폼/상세(DescriptionList·Field) · D4 공개 사이트(`lib/ui/public-site/*`) · **D5 색상 lint 자동화**(`eslint no-restricted-syntax`, .tsx 원시색 금지 + 의미색 토큰화, **금지색 0건**). 모두 청사진(닫힌 PR #27) 기반 현재 main 위 재적용. **선택 후속:** 일부 long-tail 보조화면 전면 재배치(색상은 준수)·역할별 점검.
...
> **결정됨 · M4까지 구현·강제됨.** 모듈은 `packages/module-*` 로 추출됐고, 아래 1번(경계)은 **ESLint `no-restricted-imports` 로 CI 강제**된다(`packages/**` 의 `@/*`·`@church/module-*` import 차단). 새 모듈 작업·리팩터는 이 규칙을 따른다.

1. **모듈 간 직접 결합 금지.** 모듈은 **코어만** import한다(모듈→모듈·코어→모듈 import 금지 — ESLint 강제). 모듈 간/대시보드 통합은 **코어가 노출하는 읽기 계약(`readContracts`)·코어 읽기(`@church/core/member` 등)·Postgres 뷰**를 경유한다. 모듈 내부 참조는 상대경로.
2. **모듈 소유 경계.** 각 모듈은 자기 **매니페스트**(공개 표면)와 도메인 lib(내부 서브패스)를 `packages/module-*` 로 소유한다. 코어 공유 테이블(교회·사용자·권한·`member`·`department`·`org_*` 등)만 참조 가능. **Postgres 스키마 네임스페이싱**(`finance.*` 등)·모듈별 마이그레이션 분리는 **M5(선택, 미구현)** — 현재는 공유 `drizzle/`·`public` 접두어.
