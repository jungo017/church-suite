...
- **단계: ✅ 전체 완료 — 스펙 로드맵(Phase 0~5) + 보완(Phase 6) + UX 보강(페이지네이션·디자인 시스템/테마) + 설문·보고 모듈(셀프 제출·파일첨부·xlsx 포함) + 교적 직분 연동 + 알림 실송출 잡 연결. 85 tests, CI green, 모두 main 병합.** (다음 후보: §14 외부연동 / §12 배포 — 계정·환경 필요.)
...
- **남은 외부연동/인프라(§12·§14 — 코드 인터페이스/mock까지 준비됨):** 실제 PG(온라인헌금)·국세청 전자제출(기부금영수증)·SMS/알림톡 **실채널 드라이버**(잡·프로바이더 연결 완료 — 알리고/SENS/Solapi 자격증명·드라이버만)·소셜로그인(카카오/네이버)·네이티브 모바일앱·S3(SeaweedFS) 어댑터·배포(nginx·PgBouncer·다중 인스턴스·certbot)·저장 암호화/파기정책.
