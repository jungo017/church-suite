...
| **M4-나머지** | members/finance/site/forms 를 `packages/module-*` 로(파일럿 패턴 반복). **forms 선행:** `forms`→`members`(`org`·`portal`) 결합을 코어 `readContract`(파라미터 확장) 또는 코어 중재 인터페이스로 치환. 권장: boundary 강제(dependency-cruiser / ESLint `no-restricted-imports`)를 CI에 추가(§9). | 모듈별 격리 테스트 |
