...
import type { ModuleManifest, ModuleKey } from "@/lib/core/module-contract";
import {
  visibleModules,
  registerModule,
  allModules,
...
describe("visibleModules — 전체 ∩ installed ∩ RBAC", () => {
  it("엔타이틀먼트로 설치되지 않은 모듈은 제외", () => {
    const out = visibleModules(ALL, set("assets"), ["admin"]);
    expect(out.map((m) => m.key)).toEqual(["assets"]);
  });

  it("설치 0개면 빈 배열", () => {
    expect(visibleModules(ALL, set(), ["admin"])).toEqual([]);
  });

  it("모듈 진입 권한이 없으면 제외 (member 역할 = 권한 없음)", () => {
    expect(visibleModules(ALL, set("assets", "finance"), ["member"])).toEqual([]);
  });

  it("viewer 는 읽기 권한 모듈만 보이고, 쓰기 네비는 숨김", () => {
    const out = visibleModules(ALL, set("assets", "finance"), ["viewer"]);
  ...
  it("원본 매니페스트를 변형하지 않는다(불변)", () => {
    ...
    visibleModules(ALL, set("assets"), ["viewer"]);
